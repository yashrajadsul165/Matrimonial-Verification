...
# =========================================================
    ...
    if photo is not None:
        ...
        if image is None:
            ...
            # Robust Haar cascade path for Streamlit Cloud/Linux and local Windows
            cascade_candidates = [
                os.path.join(cv2.__path__[0], "data", "haarcascade_frontalface_default.xml"),
                os.path.join(os.path.dirname(cv2.__file__), "data", "haarcascade_frontalface_default.xml"),
            ]

            cascade_path = next((path for path in cascade_candidates if os.path.exists(path)), None)

            if cascade_path is None:
                st.error("OpenCV face-detection model is missing. Please reinstall opencv-python-headless.")
                faces = ()
            else:
